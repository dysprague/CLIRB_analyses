#!/usr/bin/env bash
# Render single-panel + 3D triptych for:
#   - reproj_v4 (global, with smooth=15) — all 6 sessions
#   - R1_reproj_ft (only R1 sessions)
#   - R2_reproj_ft (only R2 sessions)
#   - R3_reproj_ft (only R3 sessions)
# All use --smooth_size 15.
set -euo pipefail
...
run_set() {
    ...
    for entry in "$@"; do
        ...
        $PY -u -m corrector.render_world_overlay --ckpt "$ckpt" --rat "$rat" \
            ...
            --single_panel --smooth_size 15 --out_subdir "$sub_sp"
    done
    for entry in "$@"; do
        rat=${entry% *}; sess=${entry#* }
        $PY -u -m corrector.render_world_3d_triptych --ckpt "$ckpt" --rat "$rat" \
            --session "$sess" --n_frames 1000 \
            --smooth_size 15 --out_subdir "$sub_trip"
    done
}

# v4 global, smooth=15, all 6 sessions
run_set corrector/checkpoints/R1R2R3_temporal_mlp_2d_reproj_v4.pt \
    R1R2R3_temporal_mlp_2d_reproj_v4_smooth15_singlepanel \
    R1R2R3_temporal_mlp_2d_reproj_v4_smooth15_3dtriptych \
    "${ALL_SESSIONS[@]}"

# R1 fine-tune, 2 R1 sessions
run_set corrector/checkpoints/R1_reproj_ft_from_v3.pt \
    R1_reproj_ft_smooth15_singlepanel \
    R1_reproj_ft_smooth15_3dtriptych \
    "${R1_SESSIONS[@]}"

# R2 fine-tune, 2 R2 sessions
run_set corrector/checkpoints/R2_reproj_ft_from_v3.pt \
    R2_reproj_ft_smooth15_singlepanel \
    R2_reproj_ft_smooth15_3dtriptych \
    "${R2_SESSIONS[@]}"

# R3 fine-tune, 2 R3 sessions
run_set corrector/checkpoints/R3_reproj_ft_from_v3.pt \
    R3_reproj_ft_smooth15_singlepanel \
    R3_reproj_ft_smooth15_3dtriptych \
    "${R3_SESSIONS[@]}"
